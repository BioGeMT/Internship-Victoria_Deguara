...
plt.figure()
for k in kmers:
    plt.plot(k['recall'], k['precision'], label=f"{k['model']}", linewidth=2)
    print(f'The average precision score for {k["model"]} is {k["ap"]}')
        
plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("Precision vs Recall Curve: AGO2_CLASH_Hejret2023", fontsize=12)
plt.xlim([0.0, 1.05])
plt.ylim([0.0, 1.05])
plt.grid(True, linestyle="--", alpha=0.5)
plt.legend(loc="lower left")

plt.savefig("precision_recall_curve_output.png", dpi=300)
plt.show()    
